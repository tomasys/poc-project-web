...
//-------------------------------------------------------------
// txId
var tx_InquireServey 	= "공지사항조회";
var tx_SaveServey 		= "공지사항등록";
var tx_DeleteServey 	= "공지사항삭제";
var tx_CheckServey 		= "공지사항확인및공지대상조회";
...
*  주요내용 : 공지사항  내역을 조회함.
*******************************************************************************/
function CRMBO000M0_OnLoadCompleted(obj)
{	
    //사용자 정의 폼 초기화 Call
	f_CommonInitForm();
}
...
function f_CommonInitForm()
{
	...
	// 콜센터 공통코드 로딩
	var x_GroupCode = Array(3);
	x_GroupCode[0] = "KM002,cob_SearchType,B";
	x_GroupCode[1] = "BO001,cob_SearchKind,A";
	x_GroupCode[2] = "BO001,cob_NoticeType,B";
...
function f_DoPost(txId)
{
	if(txId == tx_InquireServey){		
		...
		var regId 		= ds_Notice.GetColumn(ds_Notice.rowposition, "rgr_id");
		// 편집가능 모드
		if(regId == x_UserId){
			f_SetAttachButton(3);
...
//---------------------------------------------------------------
function f_CommonInquire()					//공지사항 그리드 조회
{	
	f_SetAttachButton(1);					// 첨부관련 파일 비활성
	
	var txCode = "CRMBO000O001";			// 거래코드
	ds_Notice.ClearData();					// 조회 전 Dataset 초기화
				
	// output dataset
	var outDatasets = Array(1);				// Output Dataset
	outDatasets[0] = ds_Notice.ID;
	
	var variables = Array(5);				// 입력 파라미터 Array
	variables[0] = "ip_stcal,"		+ quote(cal_Start.Value);
	variables[1] = "ip_encal,"		+ quote(cal_End.Value);
	variables[2] = "ip_ntc_type,"	+ quote(cob_SearchKind.Value);
	variables[3] = "ip_src_type,"	+ quote(cob_SearchType.Value);
	variables[4] = "ip_src_value,"	+ quote(edt_Keywords.Text);
...
function f_CommonStore()
{
	...
	var x_nowRow	= ds_Notice.rowposition;
	var x_nowState 	= ds_Notice.GetRowType(x_nowRow);
	...
	var txCode  	= "CRMBO000O002";

	var inDs        = Array(2);             	
	inDs[0]     	= ds_Notice.ID + ",U";
	inDs[1]     	= ds_NoticeSubject.ID + ",A";		
...
function f_CommonDelete()
{
	...
	var x_NowRow  	= ds_Notice.rowposition;
	var noticeId 	= ds_Notice.GetColumn(x_NowRow, "ntic_mtt_id");
	var delFileId 	= ds_Notice.GetColumn(x_NowRow, "atcg_file_id");
	...
	variables[1] 	= "ntic_mtt_id,"	+ quote(noticeId);
...
function f_AttachDelete()
{
	...
	var x_nowRow	= ds_Notice.rowposition;
	var fileID 		= ds_Notice.GetColumn(ds_Notice.rowposition, "atcg_file_id");	// 파일ID 
    s_DeleteFileAttach(fileID);													// ds_Files에 첨부삭제
    ds_Notice.SetColumn(ds_Notice.rowposition, "atcg_file_id","");
	
	var txId    	= tx_DeleteFile;
	var txCode  	= "CRMBO000O005";
...
{
	if(ds_Notice.GetRowType(ds_Notice.rowposition) == "insert"){
	...
	var x_NowRow = ds_Notice.rowposition;
	var x_rgr_id = ds_Notice.GetColumn(x_NowRow, "rgr_id");
...
function btn_BtlStartCal_OnClick(obj)
{
	cal_BltStart.DropDown();
}

function btn_BtlEndCal_OnClick(obj)
{
	cal_BltEnd.DropDown();
}
...
//   주요내용 : 공지사항 내용에 파일을 첨부한다.
//--------------------------------------------------------------
function btn_Attach_OnClick(obj)		//공지사항 첨부파일을 추가
{	
	// 파일 선택 창 출력.
	...
	if(fileId >= 0) 
	{
		ds_Notice.SetColumn(ds_Notice.rowposition, "atcg_file_id", fileId); 	// 파일아이디 추가
...
//   주요내용 : 공지사항 내용에 첨부한 파일을 첨부취소한다.
//--------------------------------------------------------------
function btn_CancelAttach_OnClick(obj)
{
	var fileId = ds_Notice.GetColumn(ds_Notice.rowposition, "atcg_file_id");	// 파일ID 
	s_CancelFileAttach(fileId);												// ds_Files에 첨부취소
	ds_Notice.SetColumn(ds_Notice.rowposition, "atcg_file_id","");				// 첨부파일ID 초기화
	f_SetAttachButton(2);
	f_setAttFileName();
}
...
//   주요내용 : 공지사항 내용중에 첨부되어 있는 파일을 삭제, 저장한다.
//--------------------------------------------------------------
...
//   주요내용 : 공지사항 내용중에 첨부되어 있는 파일을 다운로드 한다.
//--------------------------------------------------------------
function btn_FileDownload_OnClick(obj)
{
	// 개발자 정의 첨부파일 데이터 셋에 포함되어 있는 File ID 획득.
	var fileId = ds_Notice.GetColumn(ds_Notice.rowposition, "atcg_file_id"); 
...
{	
	var result = false;	
	var nowRow = ds_Notice.rowposition;
	
	if(!ds_Notice.GetUpdate() && !ds_NoticeSubject.GetUpdate()){
		alert("변경사항이 존재하지 않습니다.");	
		return result;
	}
	
	if(ds_Notice.GetDelRowCount() > 0){
		if(!Confirm("삭제된 데이터가 존재합니다. 계속진행하시겠습니까?")){
			return result;
		}
	}
	
	var chkCount = ds_NoticeSubject.CaseCount("chk='1'");
	if(chkCount < 1){
		alert("공지대상을 선택하세요.");	
		return result;
	}
	...
	var category = Array();
	//그룹코드 입력항목 체크
	category[0] =  "titl,mt_ctt,sctn,blt_strymd,blt_enymd";
	category[1] =  "'제목','내용','공지종류','공지시작일','공지종료일'";
	result = !s_Validation("2", category, ds_Notice.ID);
	
	var bltStrDt = ds_Notice.GetColumn(nowRow,"blt_strymd");
	var bltEndDt = ds_Notice.GetColumn(nowRow,"blt_enymd");
	
	if(bltStrDt > bltEndDt){
		alert("공지종료일이 공지시작일보다 빠를수 없습니다.");	
		cal_BltStart.SetFocus();
		result = false;
		return result;
	}
	
	if(!ds_Notice.GetUpdate() && ds_NoticeSubject.GetUpdate()){
		//ds_Notice.SetRowType(ds_Notice.rowposition, "update");
		ds_Notice.SetColumn(nowRow,"fnl_modr_id","99999");
	}
...
//---------------------------------------------------------------
// ds_Notice_OnRowPosChanged
//   처리영역 : ds_Notice의 줄바꿈시 Call
//   주요내용 : 줄발꿈시 권한에 따른 화면 설정
//---------------------------------------------------------------
function ds_Notice_OnRowPosChanged(obj,nOldRow,nRow)
{
	ds_Files.ClearData();
	ds_NoticeSubject.ClearData();
	...
	var x_nowState = ds_Notice.GetRowType(ds_Notice.rowposition);
	if(x_nowState == "insert"){
		f_SetAttachButton(2);
		f_CheckOpen();			// 확인자 등록, 공지대상조회
	}else{
		f_CheckOpen();			// 확인자 등록, 공지대상조회
...
{
	var x_ResoId 	= ds_Notice.GetColumn(ds_Notice.rowposition, "ntic_mtt_id");
	...
	var txCode  	= "CRMBO000O003";

	var inDs        = null;          	
	var outDs       = Array(1);
	outDs[0] 		= ds_NoticeSubject.ID;
	  
	var variables   = Array(4);  
	variables[0] 	= "ip_clsf_sctcd,'1'";				// 구분(공지사항:1, 업무자료실:2, FAQ:3, Q&A:4)
	variables[1] 	= "ip_reso_id," + quote(x_ResoId);	// 자원ID(공지사항ID,업무자료실ID,FAQ/Q&A ID)
	variables[2] 	= "ip_user_id," + quote(x_UserId);	// 사용자ID
	variables[3] 	= "ip_inq_ip,"  + quote(x_UserIp[0]);	// 접속 IP
...
{
	var x_nowRow	= ds_Notice.rowposition;
	var x_FileId 	= ds_Notice.GetColumn(x_nowRow, "atcg_file_id");
	var x_ResoId 	= ds_Notice.GetColumn(x_nowRow, "ntic_mtt_id");
	var x_rgr_id 	= ds_Notice.GetColumn(x_nowRow, "rgr_id");
...
//---------------------------------------------------------------
// grd_Notice OnCellDblClick
//   처리영역 : 확인자수 더블클릭시 호출
//   주요내용 : 확인자 목록 팝업 오픈
//---------------------------------------------------------------
function grd_Notice_OnCellDblClick(obj,nRow,nCell,nX,nY,nPivotIndex)
{
	if(nCell == 7){
		var x_ResoId 	= ds_Notice.GetColumn(ds_Notice.rowposition, "ntic_mtt_id");
...
//---------------------------------------------------------------
// 공지사항목록 이동선택 전
//   처리영역 : ds_Notice CanRowPosChange에서 Event 호출
//   주요내용 : 
//---------------------------------------------------------------
function ds_Notice_CanRowPosChange(obj,nOldRow,nRow)
{
	if(ds_Notice.GetUpdate()){
		alert("작업중인 내용이 존재합니다. 작업완료후 계속하세요.");
		return false;
...
{
	var x_atcg_file_id 	= ds_Notice.GetColumn(ds_Notice.rowposition, "atcg_file_id");
